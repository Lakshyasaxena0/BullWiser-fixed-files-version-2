...
export class DatabaseStorage implements IStorage {
  ...
  async createSubscription(subscription: InsertSubscription): Promise<Subscription> {
    const [result] = await withRetry(() =>
      db.insert(subscriptions).values(subscription).returning()
    );
    return result;
  }
  ...
  async createFeedback(feedbackData: InsertFeedback): Promise<Feedback> {
    const [result] = await withRetry(() =>
      db.insert(feedback).values(feedbackData).returning()
    );
    return result;
  }
  ...
  async createPrediction(prediction: InsertPrediction): Promise<Prediction> {
    const [result] = await withRetry(() =>
      db.insert(predictions).values(prediction).returning()
    );
    return result;
  }
  ...
  async updatePredictionStatus(id: number, isActive: boolean): Promise<void> {
    await withRetry(() =>
      db.update(predictions).set({ isActive }).where(eq(predictions.id, id))
    );
  }

  async addToWatchlist(watchlistItem: InsertWatchlist): Promise<WatchlistItem> {
    const [result] = await withRetry(() =>
      db.insert(watchlist).values(watchlistItem).returning()
    );
    return result;
  }
  ...
  async getTrainingStatus(): Promise<{ progress: number; message: string; startedAt: number } | undefined> {
    const [status] = await withRetry(() =>
      db.select().from(trainingStatus).where(eq(trainingStatus.id, 1))
    );
