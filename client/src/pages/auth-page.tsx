...
export default function AuthPage() {
  ...
  // Silently wake up the backend + Neon as soon as the page loads.
  // By the time the user fills the form, the DB is already warm.
  useEffect(() => {
    let cancelled = false;
    async function warmup() {
      for (let i = 0; i < 10; i++) {
        try {
          const res = await fetch("/api/warmup", { credentials: "include" });
          if (res.ok && !cancelled) {
            console.log("[warmup] DB is ready");
            setDbReady(true);
            return;
          }
        } catch {}
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="register">
                <form onSubmit={handleRegister} className="space-y-4">
                  ...
                  <Button type="submit" className="w-full" disabled={registerMutation.isPending || !dbReady}>
                    {registerMutation.isPending ? "Creating account..." : !dbReady ? "Waiting for DB..." : "Create Account"}
                  </Button>
                </form>
              </TabsContent>
            </Tabs>
